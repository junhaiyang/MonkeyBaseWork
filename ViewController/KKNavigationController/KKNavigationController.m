...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    //询问顶级页面是否需要手势操作，如果返回YES,则表示需要滑动手势，当前就不处理，返回NO
    if([self.topViewController respondsToSelector:@selector(gestureRecognizer:shouldReceiveTouch:)]){
        
        @try {
            NSMethodSignature *methodSignature = [self.topViewController methodSignatureForSelector:@selector(gestureRecognizer:shouldReceiveTouch:)];
            NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:methodSignature];
            [invocation setTarget:self.topViewController];
            [invocation setSelector:@selector(gestureRecognizer:shouldReceiveTouch:)];
            [invocation setArgument:&gestureRecognizer atIndex:2];
            [invocation setArgument:&touch atIndex:3];
            [invocation invoke];
...
- (void)paningGestureReceive:(UIPanGestureRecognizer *)recoginzer
{
    ...
    if (recoginzer.state == UIGestureRecognizerStateBegan) {
        ...
        if (_isMoving) {
            if(touchPoint.x - startTouch.x>30)
...
- (void)_panGestureRecognizerDidFinish:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    __block KKNavigationController *blockSelf =self;
    
    void (^completion)(BOOL) = ^(BOOL finished) {
        _isMoving = NO;
        if (finishPop) {
            
            if([blockSelf.topViewController respondsToSelector:@selector(navigationWillPopController)]){
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                [blockSelf.topViewController performSelector:@selector(navigationWillPopController)];
#pragma clang diagnostic pop
    ...
    [UIView animateWithDuration:duration animations:^{
        [blockSelf moveViewWithX:finishPop ? navigationWidth : 0];
